...
describe('formatNextRunTime', () => {
  ...
  it('returns "Today" when next run is same date in given timezone', () => {
    const next = new Date('2026-03-31T07:00:00Z');
    const result = formatNextRunTime(next, 'UTC');
  ...
  it('returns "Tomorrow" when next run is next date in given timezone', () => {
  ...
  it('renders in the correct timezone for non-UTC tasks', () => {
    // 2026-03-31T23:00:00Z is April 1 in Asia/Tokyo (UTC+9)
    const next = new Date('2026-03-31T23:00:00Z');
    const result = formatNextRunTime(next, 'Asia/Tokyo');
    expect(result).toContain('8:00 AM Asia/Tokyo');
    expect(result).toContain('Wed');
  });
});
